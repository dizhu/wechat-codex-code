...
设为前三者时会自动附加 `--ask-for-approval never`——微信端没有终端可交互审批，保留审批会让请求一直卡住。**设了非法值（如拼错成 `readonly`）会告警并 fail-safe 到最严格的 `read-only`**，而不是退回绕过模式——避免"以为开了沙箱、实际仍是完全权限"。不设置该变量才是默认的绕过模式。
