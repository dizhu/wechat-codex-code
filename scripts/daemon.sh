...
# =============================================================================
# wechat-codex-code cross-platform daemon manager
# Supports: macOS (launchd) / Linux (systemd + nohup fallback)
# =============================================================================

DATA_DIR="${HOME}/.wechat-codex-code"
...
macos_plist_label() {
  echo "com.wechat-codex-code.bridge"
}
...
macos_stop() {
  ...
  rm -f "$plist_path"
  echo "Stopped wechat-codex-code daemon (macOS launchd)"
}
...
Documentation=https://github.com/dizhu/wechat-codex-code
...
linux_direct_start() {
  ...
  echo "Starting wechat-codex-code daemon (direct mode)..."
...
linux_start() {
  if linux_systemd_available; then
    ...
    echo "Started wechat-codex-code daemon (Linux systemd)"
  else
...
linux_stop() {
  if linux_systemd_available && systemctl --user cat "${SERVICE_NAME}" &>/dev/null; then
    ...
    echo "Stopped wechat-codex-code daemon (Linux systemd)"
  else
    linux_direct_stop
  fi
}
