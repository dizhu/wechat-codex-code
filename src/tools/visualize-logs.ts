#!/usr/bin/env node
/**
 * Log Visualizer for wechat-codex-code
 *
...
import { DATA_DIR, SILENCE_MESSAGES as SILENCE_MESSAGE_LIST } from '../constants.js';

// Keepalive messages shared with the daemon (constants.ts) so the two can't drift.
const SILENCE_MESSAGES = new Set(SILENCE_MESSAGE_LIST);
...
<title>WeChat-Codex-Code Log Viewer — ${esc(date)}</title>
<style>
  :root {
    --bg: #f0f2f5;
    --card: #fff;
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .section-title.codex { background: var(--green-bg); }
...
<body>
<h1>WeChat-Codex-Code Log Viewer</h1>
...
function renderSessionCard(s: Session): string {
  ...
  // Codex full output
  const codexHtml = s.codexFullOutput
  ...
  <div class="session-body">
    ...
    <div class="section">
      <div class="section-title codex">Codex 完整输出${s.codexFullOutput ? ` <span class="char-count">${s.codexFullOutput.length} chars</span>` : ''}</div>
      ${codexHtml}
    </div>
    <div class="section">
